...
def calculate_dynamic_persona(df_filtered, pillar_name):
    ...
    pekerjaan_map = {
        ...
        5: "PNS (Bukan Guru)",
        6: "PNS (Guru)",
        7: "Pegawai/Karyawan Swasta",
        8: "Pengacara/Advokat/Notaris",
        9: "Pensiunan",
        10: "Perangkat Desa",
        11: "Polisi / Tentara",
        12: "Sedang Tidak Bekerja",
        13: "Tenaga Honorer",
        14: "Wiraswasta/Pengusaha/Pedagang"
    }
    ...
    pendapatan_map = {
        1: "Di atas Rp 25.000.000",
        2: "Menolak Memberikan Jawaban",
...
def render_persona_dashboard(df_exploded):
    ...
    <div style="font-size:14px; color:#6a738b; margin-bottom:20px;">
        <b>Klik salah satu gelembung motif</b> di bawah ini untuk membedah karakter sosio-demografi nasabah tersebut secara instan.
    </div>
    ...
    with col_kiri:
        with st.container(key="p1_card_bubble"):
            st.markdown("<div style='font-weight:800; font-size:16px; color:#1D2433; margin-bottom:10px;'>Pilar Motif Clusters (Clickable)</div>", unsafe_allow_html=True)
            ...
            for i, row in pillar_volumes.iterrows():
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=row['Pilar'],
                    hovertemplate=f"<b>{row['Pilar']}</b><br>Klik untuk melihat persona<extra></extra>"
                ))
        ...
        with st.container(key="p1_card_persona"):
            ...
            if persona_data:
                ...
                html_identity = (
                    ...
                    f'Usia: <span style="color:#6a738b;">{persona_data["usia"]} ({persona_data["usia_pct"]}%)</span>'
                    f'</div>'
                    f'<div style="font-size:13px;color:#6a738b;">Volume: <b>{persona_data["volume"]} Nasabah</b></div>'
                    '</div>'
                    '</div>'
                )
                ...
                html_socio = (
                    ...
                    'text-transform:uppercase;margin-bottom:8px;">Sosio-Demografi Dominan</div>'
                    ...
                    f'🎓 <b>Pendidikan:</b> {persona_data["pendidikan"]} ({persona_data["pendidikan_pct"]}%)<br>'

                    f'💼 <b>Pekerjaan:</b> {persona_data["pekerjaan"]} ({persona_data["pekerjaan_pct"]}%)<br>'

                    f'💰 <b>Pendapatan:</b> {persona_data["pendapatan"]} ({persona_data["pendapatan_pct"]}%)<br>'

                    f'🛒 <b>Pengeluaran:</b> {persona_data["pengeluaran"]} ({persona_data["pengeluaran_pct"]}%)'
                    '</div>'
                    '</div>'
                )
                ...
                html_empty = (
                    ...
                    '<div style="color:#98A1B3;font-weight:600;margin-top:10px;">Data persona tidak tersedia.</div>'
                    '</div>'
                )
